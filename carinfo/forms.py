...
class CarForm(forms.ModelForm):
    ...
    length = forms.IntegerField(
        label='長さ（mm）', min_value=0, required=False,
        widget=forms.NumberInput(attrs={'placeholder': '例：4460', 'class': 'form-control'})
    )
    width = forms.IntegerField(
        label='幅（mm）', min_value=0, required=False,
        widget=forms.NumberInput(attrs={'placeholder': '例：1740', 'class': 'form-control'})
    )
    height = forms.IntegerField(
        label='高さ（mm）', min_value=0, required=False,
        widget=forms.NumberInput(attrs={'placeholder': '例：1490', 'class': 'form-control'})
    )
    ...
    class Meta:
        ...
        fields = [
            ...
            'notes', 'main_photo',            # ⬅️ 用 main_photo；不再用 image
        ]
        labels = {
            'name': '車両名',
            ...
            'insurance_end_date': '保険満了日',   # ⬅️ 字段名修正
            'etc_device': 'ETC機器番号',
        ...
        widgets = {
            # 日期控件
            'first_registration_date': forms.DateInput(attrs={'type': 'date', 'class': 'form-control'}),
            ...
            # 图片上传
            'main_photo': forms.ClearableFileInput(attrs={'class': 'form-control', 'accept': 'image/*'}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            if isinstance(field.widget, (forms.TextInput, forms.NumberInput, forms.DateInput, forms.Textarea, forms.Select)):
                ...
                field.widget.attrs.setdefault('placeholder', f'例：{field.label}')
